export const mockDesigners = [
  ...
  },
  {
    ...
    specialties: ["Mens Suits", "Shirts", "Traditional"],
    portfolio: [],
    priceRange: "GHc 250-600",
  },
];
...
const defaultDesigners = [
  {
    ...
    specialty: "Bridal and Eveningwear",
    location: "Accra",
...
export function createDesignerAccount(profile) {
  ...
  const alreadyExists = designers.some((designer) => designer.email.toLowerCase() === email);
...
export function authenticateDesigner(email, password) {
  const designers = loadDesigners();
  const designer = designers.find((entry) => entry.email.toLowerCase() === email.trim().toLowerCase());
